			...
			@foreach ($ads as $ad)
				<li class="job_list_filter_item cf">
					...
					<div class="job_list_filter_item_left">
						...
						<small class="date">{{ $ad->ad->created_at }}</small>
						<h3 class="job_list_filter_item_title bold">
							<a href="{{ route('getSpecificJob', ['jid' => $ad->ad->id, 'cid' => $ad->ad->company->id]) }}">{{ $ad->ad->position }}</a>
						</h3>
						
						<div class="job_list_filter_item_info">
							<p class="location"><span class="bold">Location:</span>{{ $ad->ad->city }}</p>
							<p class="salary"><span class="bold">Salary:</span>from {{ $ad->ad->salary_from }} to {{ $ad->ad->salary_to }} {{ $ad->ad->currency }} ({{ $ad->ad->salary_type }})</p>	
						</div>
